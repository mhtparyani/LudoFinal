...
public class Game_Simulator
{
	...
	TestPlayer RY;
	TestPlayer RG;
	public string SIMULATE(Chromosome C, int noOfPlayers)
    {
		...
		if (noOfPlayers == 4) {
			RP = new TestPlayer(Board, "Red", 14, 6, totalGeetiyan);
			RB = new TestPlayer(Board, "Blue", 33, 25, totalGeetiyan);
			RY = new TestPlayer(Board, "Yellow", 52, 44, totalGeetiyan);
			RG = new TestPlayer(Board, "Green", 71, 63, totalGeetiyan);
...
public class GA
{
	...
	public void CROSS_OVER(Chromosome[] Population, int POPULATION_SIZE, int iter, int noOfPlayers)
    {
		...
		offspring1.Fitness=CALCULATE_FITNESS(offspring1,noOfPlayers);
        Population[WEAKEST_1] = offspring1;						//Deleting weakest individuals and placing the new offsprings
        
		offspring2.Fitness=CALCULATE_FITNESS(offspring2,noOfPlayers);
	...
	public int CALCULATE_FITNESS(Chromosome C, int noOfPlayers)
    {
        int Fitness = 0;
        Game_Simulator G= new Game_Simulator();
        for (int i = 1; i <= 100; i++)
			if (G.SIMULATE(C, noOfPlayers) == "Red")
                Fitness++;
        
		return Fitness;
    }

	public Chromosome RUN_GENETIC_ALGORITHM(int noOfPlayers)
    {
        ...
        for (int i = 0; i < POPULATION_SIZE; i++)
        {
			...
			Population[i].Fitness = CALCULATE_FITNESS(Population[i], noOfPlayers);
        }
        ...
        {
			CROSS_OVER(Population, POPULATION_SIZE, i,noOfPlayers);
        }
...
public struct Player{
	...
	public bool MoveGeeti(int dice,Square []Board,int n,ref Player RP,ref Player RB,ref Player RY,ref Player RG){
		...
		Vector3 sizeOriginal = new Vector3 (0.03f, 0.03f, 0.03f);
		if(Geeti[n-1].index==-1){
			if(dice==6){
				...
				Vector3 p=GameObject.Find(Board[Geeti[n-1].index].waypoint).GetComponent<Collider>().gameObject.transform.position;
				Geeti[n-1].ChangePosition(p);
				RP.Geeti [0].transform.localScale = sizeOriginal;
				RP.Geeti [1].transform.localScale = sizeOriginal;
				RP.Geeti [2].transform.localScale = sizeOriginal;
				RP.Geeti [3].transform.localScale = sizeOriginal;

				RB.Geeti [0].transform.localScale = sizeOriginal;
				RB.Geeti [1].transform.localScale = sizeOriginal;
				RB.Geeti [2].transform.localScale = sizeOriginal;
				RB.Geeti [3].transform.localScale = sizeOriginal;

				RY.Geeti [0].transform.localScale = sizeOriginal;
				RY.Geeti [1].transform.localScale = sizeOriginal;
				RY.Geeti [2].transform.localScale = sizeOriginal;
				RY.Geeti [3].transform.localScale = sizeOriginal;

				RG.Geeti [0].transform.localScale = sizeOriginal;
				RG.Geeti [1].transform.localScale = sizeOriginal;
				RG.Geeti [2].transform.localScale = sizeOriginal;
				RG.Geeti [3].transform.localScale = sizeOriginal;
				return true;
			}
			else{
			...
			if(i==dice+1){										//Successful Complete Turn
				...
				Vector3 p2=GameObject.Find(Board[Geeti[n-1].index].waypoint).GetComponent<Collider>().gameObject.transform.position;
				Geeti[n-1].ChangePosition(p2);
				RP.Geeti [0].transform.localScale = sizeOriginal;
				RP.Geeti [1].transform.localScale = sizeOriginal;
				RP.Geeti [2].transform.localScale = sizeOriginal;
				RP.Geeti [3].transform.localScale = sizeOriginal;
...
public class Gamer : MonoBehaviour {
   ...
   //public Text Texttq;
	...
	public GameObject reddQ;
	public GameObject greeenQ;
	public GameObject blueeQ;
		...
		if (isWaiting && !isError) {
			...
			if (noOfPlayers.Equals (4)) {
				if (dice < 6 && ((turn == 1 && RP.Nikli == 0) || (turn == 2 && RB.Nikli == 0) || (turn == 3 && RY.Nikli == 0) || (turn == 4 && RG.Nikli == 0))) {
					...
					if (turn == 1) {
						//redd.GetComponent<Dice> ().canRoll = true;
						bluee.GetComponent<Dice> ().canRoll = true;
						//yellow.GetComponent<Dice> ().canRoll = true;
						//greeen.GetComponent<Dice> ().canRoll = true;
					}else if (turn == 2) {
						//redd.GetComponent<Dice> ().canRoll = true;
						//bluee.GetComponent<Dice> ().canRoll = true;
						yellow.GetComponent<Dice> ().canRoll = true;
				...
				if (dice < 6 && ((turn == 1 && RP.Nikli == 0) || (turn == 2 && RB.Nikli == 0) || (turn == 3 && RY.Nikli == 0))) {
				...
				if (dice < 6 && ((turn == 1 && RP.Nikli == 0) || (turn == 2 && RY.Nikli == 0))) {
...
//		}*/
	...
	void DisplayInfo(int noOfPlayers){
		...
		if (noOfPlayers.Equals(4)) {
			if(turn==1){
				...
				reddQ.SetActive(true);
				greeenQ.SetActive(false);
				blueeQ.SetActive(false);
				yellowQ.SetActive(false);
				/*GameObject.Find ("_Blue1").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Blue2").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Blue3").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Blue4").GetComponent<Collider> ().enabled = false;

				GameObject.Find ("_Red1").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Red2").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Red3").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Red4").GetComponent<Collider> ().enabled = true;

				GameObject.Find ("_Green1").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Green2").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Green3").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Green4").GetComponent<Collider> ().enabled = false;

				GameObject.Find ("_Yellow1").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Yellow2").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Yellow3").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Yellow4").GetComponent<Collider> ().enabled = false;*/


				//GameObject.Find("T_Turn").GetComponent<GUIText>().material.color=Color.red;

				//GameObject.Find("T_Turn").GetComponent<GUIText>().text="RED's Turn";
				//GameObject.Find("T_Nikli").GetComponent<GUIText>().text="Nikli: "+RP.Nikli.ToString();
				//GameObject.Find("T_Pug").GetComponent<GUIText>().text="Pug: "+RP.Pug.ToString();

				if (RP.canGoHome){
					//GameObject.Find("T_canGoHome").GetComponent<GUIText>().material.color=Color.green;
					//	GameObject.Find("T_canGoHome").GetComponent<GUIText>().text="Can enter Home";
				}
				else{
					//	GameObject.Find("T_canGoHome").GetComponent<GUIText>().material.color=Color.red;
					//	GameObject.Find("T_canGoHome").GetComponent<GUIText>().text="Cannot enter Home";
				...
				reddQ.SetActive(false);
				greeenQ.SetActive(false);
				blueeQ.SetActive(true);
				yellowQ.SetActive(false);
				/*GameObject.Find ("_Blue1").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Blue2").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Blue3").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Blue4").GetComponent<Collider> ().enabled = true;

				GameObject.Find ("_Red1").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Red2").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Red3").GetComponent<Collider> ().enabled = false;
				GameObject.Find ("_Red4").GetComponent<Collider> ().enabled = false;
				...
				//GameObject.Find("T_Nikli").GetComponent<GUIText>().text="Nikli: "+RB.Nikli.ToString();
				//GameObject.Find("T_Pug").GetComponent<GUIText>().text="Pug: "+RB.Pug.ToString();
				...
				GameObject.Find ("_Yellow1").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Yellow2").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Yellow3").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Yellow4").GetComponent<Collider> ().enabled = true;*/
				//	GameObject.Find("T_Turn").GetComponent<GUIText>().material.color=Color.yellow;

				//	GameObject.Find("T_Turn").GetComponent<GUIText>().text="YELLOW's Turn";
				//GameObject.Find("T_Nikli").GetComponent<GUIText>().text="Nikli: "+RY.Nikli.ToString();
				//GameObject.Find("T_Pug").GetComponent<GUIText>().text="Pug: "+RY.Pug.ToString();
				...
				GameObject.Find ("_Green1").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Green2").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Green3").GetComponent<Collider> ().enabled = true;
				GameObject.Find ("_Green4").GetComponent<Collider> ().enabled = true;
				...
				//	GameObject.Find("T_Nikli").GetComponent<GUIText>().text="Nikli: "+RG.Nikli.ToString();
				//	GameObject.Find("T_Pug").GetComponent<GUIText>().text="Pug: "+RG.Pug.ToString();
	...
	void Start () {
		...
		GA ga=new GA();
		Best=new Chromosome();
		Best=ga.RUN_GENETIC_ALGORITHM(noOfPlayers);
		...
		if (noOfPlayers.Equals (2)) {
			...
			GameObject.Find ("_Blue1").SetActive (false);
			GameObject.Find ("_Blue2").SetActive (false);
			GameObject.Find ("_Blue3").SetActive (false);
			GameObject.Find ("_Blue4").SetActive (false);
			GameObject.Find ("_Green1").SetActive (false);
			GameObject.Find ("_Green2").SetActive (false);
			GameObject.Find ("_Green3").SetActive (false);
			GameObject.Find ("_Green4").SetActive (false);
		}else if(noOfPlayers.Equals (3)){
					...
					if(noOfPlayers.Equals(4)){
						if(hit.collider.gameObject.name.StartsWith("_R") && turn==1){
							...
							if(hit.collider.gameObject.name=="_Blue1")
								response=RB.MoveGeeti(dice,Board,1,ref RP,ref RB,ref RY,ref RG);

							else if(hit.collider.gameObject.name=="_Blue2")
								response=RB.MoveGeeti(dice,Board,2,ref RP,ref RB,ref RY,ref RG);

							else if(hit.collider.gameObject.name=="_Blue3")
								response=RB.MoveGeeti(dice,Board,3,ref RP,ref RB,ref RY,ref RG);

							else if(hit.collider.gameObject.name=="_Blue4")
								response=RB.MoveGeeti(dice,Board,4,ref RP,ref RB,ref RY,ref RG);

							if(dice==6)
								isWaiting=true;

							if (!response) {					//Move not possible. isWaiting is set o that dice is not rolled until a valid MOVE is made
								if (RB.isFinishOrBeat) {
									isWaiting = true;
								} else {
									isWaiting = false;
								}
							...
							if(hit.collider.gameObject.name=="_Green1")
								response=RG.MoveGeeti(dice,Board,1,ref RP,ref RB,ref RY,ref RG);

							else if(hit.collider.gameObject.name=="_Green2")
								response=RG.MoveGeeti(dice,Board,2,ref RP,ref RB,ref RY,ref RG);

							else if(hit.collider.gameObject.name=="_Green3")
								response=RG.MoveGeeti(dice,Board,3,ref RP,ref RB,ref RY,ref RG);

							else if(hit.collider.gameObject.name=="_Green4")
								response=RG.MoveGeeti(dice,Board,4,ref RP,ref RB,ref RY,ref RG);

							if(dice==6)
								isWaiting=true;

							if (!response) {					//Move not possible. isWaiting is set o that dice is not rolled until a valid MOVE is made
								if (RG.isFinishOrBeat) {
									isWaiting = true;
								} else {
									isWaiting = false;
								}
						...
						else if(hit.collider.gameObject.name.StartsWith("_Y") && turn==2){
